...
object OtherFeatureNavGraph {
    ...
    fun graph(
        ...
        with(navGraphBuilder) {
            navigation(
                route = miscellaneous.OtherFeatureNavGraph.ROUTE,
                startDestination = miscellaneous.OtherFeatureNavGraph.HOME_ROUTE
            ) {
                composable(route = miscellaneous.OtherFeatureNavGraph.HOME_ROUTE) {
                    TopBarDecorator(
                        enableNavigation = miscellaneous.OtherFeatureNavGraph.showNavigationIcon.collectAsState().value,
                ...
                composable(route = miscellaneous.OtherFeatureNavGraph.ABOUT_US_SCREEN) {
                    TopBarDecorator(
                        enableNavigation = miscellaneous.OtherFeatureNavGraph.showNavigationIcon.collectAsState().value,
                        onExitRequest = onExitRequest,
                        title = "About Us"
                ...
                composable(route = miscellaneous.OtherFeatureNavGraph.EVENT_GALLERY_SCREEN) {
                    TopBarDecorator(
                        enableNavigation = miscellaneous.OtherFeatureNavGraph.showNavigationIcon.collectAsState().value,
                        onExitRequest = onExitRequest,
                        title = "Event Gallery"
                ...
                composable(route = miscellaneous.OtherFeatureNavGraph.MESSAGE_FROM_VC_SCREEN) {
                    TopBarDecorator(
                        enableNavigation = miscellaneous.OtherFeatureNavGraph.showNavigationIcon.collectAsState().value,
                        onExitRequest = onExitRequest,
